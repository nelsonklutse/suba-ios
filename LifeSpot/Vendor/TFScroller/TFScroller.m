...
-(void)scrollViewInitialisation{
	...
	[self removeAllSubViews:mScrollView];
...
- (void)loadScrollViewWithPage:(NSNumber*)num
{
    ...
    DLog(@"Load scrollview with page number - %i",page);
    ...
    if (imageView == NULL)
    {
		...
		imageView = [[MagazineTeaserView alloc] initWithFrame:CGRectMake(0, 0, mWidthPage, self.mScrollView.frame.size.height) andImageURL:[self.mDelegate tfScroller:self viewForIndex:(page%mActualPages)] width:mWidthPage height:mFrame.size.height];
...
-(NSMutableArray*)buttonsArray
{
	...
	for (unsigned int i = 0; i < mTotalPages; i++) {
		...
		MagazineTeaserView *tempImage = [[MagazineTeaserView alloc] initWithFrame:CGRectMake(0, 0, mWidthPage, self.mScrollView.frame.size.height) andImageURL:[self.mDelegate tfScroller:self viewForIndex:(i%mActualPages)] width:250 height:200];
