...
- (id)initWithFrame:(CGRect)frame andRemoteURL:(NSURL *)remoteUrl {
    if (self = [super initWithFrame:frame]) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            [selfDelegate sd_setImageWithURL:remoteUrl completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType,NSURL *imageURL) {
                if (!error && image) {
...
- (UILabel*)captionLabelWithPlainText:(NSString *)plainText
    ...
    captionSize = [plainText sizeWithAttributes:@{NSFontAttributeName:
        [UIFont systemFontOfSize:MAXFLOAT]}];
