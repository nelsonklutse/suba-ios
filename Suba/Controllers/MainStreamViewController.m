...
-(void)unWindToMainStream:(UIStoryboardSegue *)segue
{
    if([segue.identifier isEqualToString:@"JoinAnotherStream"]){
        ...
        Branch *branch = [Branch getTestInstance]; //:kBRANCH_API_KEY];
...
- (void)canWeShowPhotoFromTag
{
    Branch *branch = [Branch getTestInstance];
    NSDictionary *tagParams = [branch getFirstReferringParams];
    
    DLog(@"see photo from tag: %@",tagParams);
    
    if (tagParams[@"tagId"]) {
        // We need to show a tagged photo
        NSString *streamId = tagParams[@"streamId"];
        NSString *photoURL = tagParams[@"photoURL"];
        
        
        [self performSegueWithIdentifier:kPhotosStreamSegue sender:@{@"spotId": streamId, @"photoURL":photoURL}];
        
        [branch logout];
        
        
        [AppHelper matchTagToUser:@{@"tagId" : tagParams[@"tagId"], @"userId" : [AppHelper userID]} completion:^(id results, NSError *error) {
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    /*Branch *branch = [Branch getTestInstance]; //:kBRANCH_API_KEY];
    DLog(@"branch latest - %@\nbranch first - %@",[branch getLatestReferringParams],[branch getFirstReferringParams]);*/
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    // If we're coming from a tag
    [self canWeShowPhotoFromTag];
...
- (IBAction)enterSubaWithFullName:(UIButton *)sender
{
    ...
    [User updateFullName:requestData completion:^(id results, NSError *error) {
        if (error) {
            ...
            [FBAppEvents logEvent:@"Full_Name_Updated"];
...
-(void)addSpotToAllSpotsStream:(NSDictionary *)spotDetails
{
    [FBAppEvents logEvent:@"Stream_Added_To_All_Stream"];
...
- (void)loadStreamInfo:(NSString *)streamId
{
    [Spot fetchSpotInfo:streamId completion:^(id results, NSError *error) {
        if (error) {
            DLog(@"Error - %@",error);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        [FBAppEvents logEvent:@"Stream_Selected_Nearby"];
        spotsToDisplay = self.nearbyStreams;
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:kPhotosStreamSegue]) {
        if ([segue.destinationViewController isKindOfClass:[PhotoStreamViewController class]]){
            ...
            }else if([sender isKindOfClass:[NSDictionary class]] && sender[@"photoURL"]){
                
                photosVC.photoToShow = sender[@"photoURL"];
                photosVC.shouldShowPhoto = YES;
                photosVC.spotID = sender[@"spotId"];
                
                DLog(@"setting tag params");
                
                //photosVC.spotInfo = sender;
                
            }else if([sender isKindOfClass:[NSDictionary class]] && sender[@"photoURLs"]){
