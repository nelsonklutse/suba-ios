...
- (BOOL)application:(UIApplication *)application
  ...
  sourceApplication:(NSString *)sourceApplication
    ...
    if ([[Branch getTestInstance] handleDeepLink:url]) {
        return YES;
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    if (IS_OS_8_OR_LATER){
        if ([application respondsToSelector:@selector(registerUserNotificationSettings:)]) {
            ...
            UIRemoteNotificationType remoteNotifTypes = [[UIApplication sharedApplication] enabledRemoteNotificationTypes];
            
            if (remoteNotifTypes == UIRemoteNotificationTypeNone) {
    ...
    Branch *branch = [Branch getTestInstance]; //:@"key_test_mlgDMIDYz9YRGFu7KiKPChkhtEpiLX0b"];
    //Branch *branch = [Branch getInstance:@"key_live_hhmp1dtx66Q3FdbIqb65Yfdjojpuri84"];
    ...
    [branch initSessionWithLaunchOptions:launchOptions andRegisterDeepLinkHandler:^(NSDictionary *params, NSError *error) {
        if (!error) {
            ...
            if (params[@"tagId"]) {
                ...
                DLog(@"Show the tagged photo");
                [self presentPopUpOnTopMostViewController];
                
            }else{
                if (params[@"streamId"] || [ [[AppHelper getInviteParams] lastObject] count] > 0) {
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    [FBAppEvents logEvent:@"App_Started_From_Background"];
...
- (void)application:(UIApplication*)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
{
    ...
    [[SubaAPIClient sharedInstance] POST:REGISTER_DEVICE_TOKEN_URL
        ...
        [FBAppEvents logEvent:@"User_Turned_On_Push_Notification" parameters:@{@"user": userName}];
        ...
        [FBAppEvents logEvent:@"Error_Registering_For_Push_Notification" parameters:@{@"user": userName}];
    }];
}
...
-(void)application:(UIApplication *)application performFetchWithCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler
{
    [self.viewControllerForRefresh fetchNearbyStreamsInBackgroundWithCompletion:^(BOOL didReceiveNewStreams) {
        if (didReceiveNewStreams){
            DLog(@"Fetch new data");
            [FBAppEvents logEvent:@"Background_Fetch" withParameters:@{@"timestamp":[NSDate date]}];
...
- (void)setUpPopView:(UIView *)rootView
{
    ...
    if (![rootView viewWithTag:4]) { // Only add this view if there's no view showing
        ...
        NSDictionary *latestParams = [[Branch getTestInstance] getLatestReferringParams];
        DLog(@"Latest params: %@",latestParams);
        
      if ([pendingStreamInvites count] > 0 || latestParams[@"tagId"]){
        ...
        if ([_topViewController isKindOfClass:[SubaTutorialController class]] ||
            [_topViewController isKindOfClass:[SBWelcomeController class]]) {
            ...
            if (latestParams[@"tagId"]) {
                // We're showing a tag
                [logInButton setTitle:@"Log in to see photo" forState:UIControlStateNormal];
                [signUpButton setTitle:@"Sign up to see photo" forState:UIControlStateNormal];
            }
        ...
        // Set info and image for pop up
          ...
          if (latestParams[@"tagId"]) {
              ...
              [popUpImageView setImageWithURL:[NSURL URLWithString:latestParams[@"senderPhoto"]] placeholderImage:[UIImage imageNamed:@"anonymousUser"]];
              
              popUpMessgaeLabel.text = [NSString stringWithFormat:@"%@ tagged you in a photo",
                                        latestParams[@"sender"]];
              [popUpMessgaeLabel sizeToFit];
              
              popUpTitleLabel.text = @"You've been tagged";
              [popUpTitleLabel sizeToFit];
              
              [joinStreamButton setTitle:@"See photo" forState:UIControlStateNormal];
              [joinStreamButton addTarget:self action:@selector(showTaggedPhoto:) forControlEvents:UIControlEventTouchUpInside];
          }else{
              ...
              popUpTitleLabel.text = @"You've got 1 pending invite";
              [popUpTitleLabel sizeToFit];
...
- (void)showTaggedPhoto:(UIButton *)sender
{
    //1. PhotoURL   2. StreamId
    Branch *branch = [Branch getTestInstance];
    
    NSDictionary *tagParams = [branch getLatestReferringParams];
    
    DLog(@"Just about to show the tagged photo with tag: %@",tagParams);
    ...
    if ([tagParams count] > 0) {
        ...
        //NSString *streamName = tagParams[@"streamName"];
        NSString *streamId = tagParams[@"streamId"];
        NSString *photoURL = tagParams[@"photoURL"];
        
        NSDictionary *info = @{@"spotId":streamId,@"photoURL":photoURL};
        ...
        if ([_topViewController isKindOfClass:[MainStreamViewController class]]) {
            ...
            [mainViewController performSegueWithIdentifier:kPhotosStreamSegue sender:info];
            ...
            UserProfileViewController *userViewController = (UserProfileViewController *)_topViewController;
                ...
                [userViewController performSegueWithIdentifier:kFromUserSpotsToPhotosStreamSegue sender:info];
            DLog(@"Yet to complete Userprofile VC");
            ...
            [activityViewController performSegueWithIdentifier:@"ACTIVITY_PHOTO_STREAM" sender:info];
        ...
        if ([_topViewController isKindOfClass:[MainStreamViewController class]]) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:kUserReloadStreamNotification object:nil];
                ...
                [userViewController joinSpot:streamCode data:inviteInfo completion:^(id results, NSError *error){
                    ...
                    [sender sizeToFit];
                    if (!error){
                        [userViewController performSegueWithIdentifier:kFromUserSpotsToPhotosStreamSegue sender:inviteInfo];
...
-(IBAction)logIn:(UIButton *)sender
{
    ...
    if ([_topViewController isKindOfClass:[SBWelcomeController class]]){
        
        SBWelcomeController *welcomeScreen = (SBWelcomeController *)_topViewController;
        
        [welcomeScreen handleGetStarted];
        
        //[firstScreen performSegueWithIdentifier:@"LogInScreen" sender:nil];
        
    }else if ([_topViewController isKindOfClass:[SubaTutorialController class]]){
        SubaTutorialController *welcomeScreen = (SubaTutorialController *)_topViewController;
        
        [welcomeScreen performSegueWithIdentifier:@"LogInScreen" sender:nil];
...
-(IBAction)signUp:(UIButton *)sender
{
    if ([_topViewController isKindOfClass:[SBWelcomeController class]]){
        ...
        [welcomeScreen showSignUpOptions];
